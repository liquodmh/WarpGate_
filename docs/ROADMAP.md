...
Deterministic traffic-trace replay, p50/p95/p99 end-to-end latency, SLA miss rate, queue depth, batch-size distribution and throughput under steady/bursty/mixed-SLA traffic. Synthetic traces are supported alongside measured batch-latency CSV profiles. The built-in profiler currently calibrates against CPU HNSW; Gate 4 will feed the same path with measured GPU timings.
